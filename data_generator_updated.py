...
def data_generator(ap_x_origin, ap_y_origin):
    ...
    for i in range(60):  # adding mobile devices
        ...
        device_details['startBuffer'] = random.choice(random.sample(range(300,7300),1))
        device_details['timeDelta'] = random.choice(random.sample(range(300,2400),1))
        devices.append(device_details)
    ...
    for device in devices:
        if (device['device_type'] == 'skimmer'):
            for i in range(10800):
                ...
                device_location['confidenceFactor'] = confidenceFactor(device_location['distance'])
                device_location['deviceType'] = device['device_type']
                data['deviceLocationUpdate'] = device_location
                response_list.append(data)
        else:
            for i in range(device['timeDelta']):
                data = {}
                device_location = {}
                current_datetime = datetime.now() + timedelta(seconds=device['startBuffer'])
                incremented_datetime = str(
                    current_datetime + timedelta(seconds=i))
                datatime_object = datetime.strptime(
                    incremented_datetime, '%Y-%m-%d %H:%M:%S.%f')
    ...
    with open('data - ' + '.csv', 'w',) as csvfile:  # generating csv
...
def confidenceFactor(distance):
    return round((100 - ((distance/235) * 100)))
